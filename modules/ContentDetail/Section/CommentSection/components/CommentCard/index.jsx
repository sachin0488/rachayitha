...
const CommentCard = ({ item, sortBy, contentType }) => {
  const { mutate } = useCommentLikeService({
  ...
  return (
    <Root className={isReplyOpen ? 'replyOpen' : ''}>
      ...
      {isReplyOpen && (
        <ReplySection commentId={item?.commentId} setCommentCount={setCommentCount} contentType={contentType} contentId={item?.contentId} />
      )}
    </Root>
  )
}
