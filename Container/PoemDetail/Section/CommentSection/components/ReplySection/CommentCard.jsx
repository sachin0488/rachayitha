...
import { useLikePoemCommentAPI } from 'Container/PoemDetail/api/poemDetail.hook'

const CommentCard = ({ item, parentCommentId }) => {
  const [likes, setLikes] = useState(item?.like_count)
  const { handleLikePoemComment } = useLikePoemCommentAPI({
    poemId: item?.poem_id,
    commentId: item?.id,
  ...
  return (
    <Root className={isReplyOpen ? 'replyOpen' : ''}>
      ...
      <ActionList>
        <StyledButton startIcon={<ThumbUpRoundedIcon />} onClick={handleLikePoemComment}>
          {likes}
        </StyledButton>
        <StyledButton
